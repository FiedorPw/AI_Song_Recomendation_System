...
if __name__ == '__main__':
    ...
    def create_and_fit_model():
        ...
        X_train, X_test = train_test_split(user_table, test_size=0.3, random_state=1234)
        X_train_clean = X_train.drop(columns=['user_id', 'index'], errors='ignore')
        pca = PCA(n_components=20)  # Zredukuj do 20 głównych składowych
        X_train_clean = pca.fit_transform(X_train_clean)
        knn = KMeans(
            n_clusters=5,
            init='k-means++',
            n_init=20,
        ...
        # Dopasowanie modelu
        knn.fit(X_train_clean)
        X_train['cluster'] = knn.labels_
        # cluster_means = X_train.groupby('cluster').mean()
        # print(cluster_means)
        return knn, X_train, X_test, pca, X_train_clean

    def recommend_tracks(user_id, X_train, X_test, complete_table_train, knn):
        # Przygotowanie danych użytkownika do predykcji
        user_data = X_test[X_test['user_id'] == user_id].drop(['user_id', 'index'], axis=1)
        user_data = pca.transform(user_data)
        tracks_played = X_test.loc[X_test['user_id'] == user_id, 'tracks_played_count'].values[0]
        ...
        cluster_users = X_train[X_train['cluster'] == user_cluster[0]]
        complete_table_train_cluster = complete_table_train[complete_table_train['user_id'].isin(cluster_users["user_id"].unique())]
        # Zbierz najczęściej słuchane utwory w tym klastrze
        top_tracks = complete_table_train_cluster['track_id'].value_counts().head(n_recommendations).index.tolist()
        return top_tracks

    def precision_recall(user_id, recommendations, complete_table_test):
        actual_tracks = complete_table_test.loc[complete_table_test['user_id'] == user_id, 'track_id'].tolist()
    ...
    def plot_elbow(X, max_clusters=100):
        inertias = [] #mierzy sumę kwadratów odległości próbek od najbliższego środka klastra.
    ...
    knn, X_train, X_test, pca, X_train_clean = create_and_fit_model()

    test_user_ids = X_test['user_id'].unique()
    user_id = test_user_ids[1]  # ID użytkownika do analizy
    print("Wybrany użytkownik do testowania:", user_id)
    # Rekomendacje
    train_user_ids = X_train['user_id'].unique()
    test_user_ids = X_test['user_id'].unique()
    # Filtrowanie complete_table dla użytkowników z train i test split
    complete_table_train = complete_table[complete_table['user_id'].isin(train_user_ids)]
    complete_table_test = complete_table[complete_table['user_id'].isin(test_user_ids)]
    recommended_tracks = recommend_tracks(user_id, X_train, X_test, complete_table_train, knn)
    precision, recall = precision_recall(user_id, recommended_tracks, complete_table_test)
    print(f"Precision: {precision:.2f}, Recall: {recall:.2f}")

    #ocena modelu - wykres lokciowy
    # Wywołanie funkcji dla danych treningowych
    plot_elbow(X_train_clean, max_clusters=100)

    silhouette_scores = []
    for k in range(2, 50):
        kmeans = KMeans(n_clusters=k, random_state=1234).fit(X_train_clean)
        score = silhouette_score(X_train_clean, kmeans.labels_)
    ...
    def average_precision_recall(X_test, complete_table_test, knn):
        precision_list = []
        recall_list = []
    ...
    # Obliczanie średniej precyzji i recall dla zbioru testowego
    avg_precision, avg_recall = average_precision_recall(X_test, complete_table_test, knn)
    print(f"Średnia precyzja: {avg_precision:.2f}")
